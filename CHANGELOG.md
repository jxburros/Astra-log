...
## 2026-04-21 - GitHub Copilot
- **Fix [P1]: Clear Notes freezes the page** (`src/components/ScratchPad.tsx`): Removed `window.confirm()` from the Clear Notes handler. The Scratch Pad is session-only and zero-persistence by design, so notes are now cleared immediately without a blocking native dialog. This unblocks automated CDP sessions and eliminates the renderer freeze.
- **Fix [P1]: Tour step 2 "Next" button off-screen on short viewports** (`src/components/WorkspaceTour.tsx`): Rewrote `tooltipStyle()` to clamp tooltip position inside the viewport for all sides. For the 'bottom' case, the card now automatically flips above the spotlight when there is insufficient room below (e.g., tall Preview panel at 739 px viewport height). Falls back to centering in the viewport when there is also no room above.
- **Fix [P2]: Terminal panel overflows left edge at ~1040 px viewport width** (`src/App.tsx`): Added a `useEffect` that auto-collapses the terminal panel in the standard layout when the viewport narrows below 1080 px (checked on mount and on every resize). This also fixes the related overflow that clips the AI Review button's subtitle (#4).
- **Fix [P2]: xterm.js `TypeError: Cannot read properties of undefined (reading 'dimensions')` on panel toggle** (`src/components/TerminalComponent.tsx`): Deferred the initial `fitAddon.fit()` call into `requestAnimationFrame` and added an `offsetWidth > 0` guard. The `ResizeObserver` callback also now only calls `fit()` inside `requestAnimationFrame` when the container has non-zero dimensions, and wraps the call in a try/catch to swallow any remaining Viewport race errors.
- **Fix [P2]: Bullets mode retroactively re-formats all existing content** (`src/components/ScratchPad.tsx`): Replaced the `onChange` full-content normalization with an `onKeyDown` handler that intercepts Enter and inserts `\n- ` only for new lines. Existing content (timestamps, headings, blank lines) is never modified when toggling bullets mode on.
- **Fix [P2]: Typing in bullets mode splits characters across lines** (`src/components/ScratchPad.tsx`): Eliminated the React state rewrite on every `onChange` that was causing cursor-position loss and character interleaving. The textarea value is now updated directly with no normalization pass.
- **Fix [P3]: Artifact Export button subtitle understates supported formats** (`src/components/ChatPanel.tsx`): Updated subtitle from `MD/PDF/TXT` to `MD/PDF/HTML/JSON/TXT` to reflect all five export formats available in the modal.
- **Fix [P3]: Error responses included in Artifact Export preview** (`src/lib/exportUtils.ts`): Added `isErrorMessage()` helper that identifies assistant messages starting with `"Error:"`. These are now filtered out of `parseArtifactSections`, `extractFixItems`, and `formatConversationForAI` so error strings (e.g. "API key is missing") are never included in exported artifacts.
- **Fix [P3]: Footer credit renders inside top header area** (`src/App.tsx`): Moved the "Created by Jeffrey Guntly · © JX Holdings, LLC" attribution from below the header (where it consumed top-chrome real estate) to a true footer at the very bottom of the viewport, consistent with standard attribution placement.
- **Fix [P3]: Settings gear gives no affordance when API key is not configured** (`src/App.tsx`): Added a small amber dot badge to the Settings gear icon when the current provider is not Local/Ollama and no API key is set. This guides first-time users who hit the "API key is missing" error directly to the Settings panel.
...
- **Fix: Silent zip drop while session is busy** (`src/App.tsx`): When a zip is dragged and dropped while a session is already running, a clear terminal message now informs the user to press "New Project" first rather than silently swallowing the drop.
- **Fix: `iframeKey` not incremented on New Project** (`src/App.tsx`): Added `setIframeKey(k => k + 1)` in `performStartOver` to guarantee a fully fresh iframe context between sessions.
- **Fix: Grok policy check extended to `customInstructions`** (`src/App.tsx`, `src/components/SettingsModal.tsx`): The Grok detection regex now also scans the Custom Instructions field. Previously a user could bypass the policy by embedding a Grok endpoint there; `handleSaveAttempt`, `updateGrokDetection`, and `handleSaveSettings` are all updated, and the Custom Instructions textarea now calls `updateGrokDetection` on every keystroke.
- **Fix: ZIP root-prefix detection is now order-independent** (`src/lib/zipParser.ts`): Replaced the previous logic that inspected `files[0]` (which relied on JSZip's unspecified iteration order) with a set-based approach that collects all distinct top-level path segments across every file, ensuring correct detection regardless of entry order.
- **Fix: SSRF / scheme injection in Ollama URL** (`server.ts`): Added `URL` parsing and scheme validation in the `/api/models` proxy endpoint — only `http://` and `https://` are allowed; other schemes (e.g. `file://`) are rejected with a 400 error.
- **Fix: `peerDependencies` now scanned by Containment Scan** (`src/lib/containmentScan.ts`): Added `peerDependencies` to the dependency merge so packages loaded via pnpm (which installs peer deps) are also checked for non-registry sources.
- **Fix: Stale model ID on Ollama URL change** (`src/components/SettingsModal.tsx`): `handleLocalUrlChange` now passes an empty string for the current model ID when triggering a fresh model discovery, so the first available model is correctly selected from the newly reachable Ollama instance.
- **Chore: Removed unused `@google/genai` dependency** (`package.json`): The package was listed as a production dependency but never imported; all Gemini calls use raw `fetch`. Removing it shrinks the dependency surface.
- **Chore: Fixed placeholder `authors` in `Cargo.toml`** (`src-tauri/Cargo.toml`): Changed `"you"` to `"Jeffrey Guntly / JX Holdings, LLC"` so the compiled binary and installer metadata carry the correct attribution.
